...
export const Swiper: React.FC<IProps> = (props: IProps) => {
    ...
    const onEnd = async (mx: number, my: number) => {
        ...
        if (d === 'right' || d === 'left') {
            ...
            tl.to(target.current, {
                x:
                    (mx - x.current > 0 ? 1 : -1) * (props.throwLimit ?? 3000) +
                    'px',
                y:
                    (my - y.current > 0 ? 1 : -1) *
            ...
            tl.to(target.current, {
                x:
                    ...
                    'px',
                y:
                    (my - y.current > 0 ? 1 : -1) * (props.throwLimit ?? 3000) +
                    'px',
                duration: 0.5,
                ease: 'power1.in',
            });
    ...
    useEffect(() => {
        ...
        if (info !== undefined) mid.current = (info.top + info.bottom) / 2;
